...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Lưu thông tin user vào localStorage để các trang khác lấy dùng
        localStorage.setItem("user_session", JSON.stringify(userData));
        toast.success(`Chào mừng ${userData.fullName} trở lại!`);
        setTimeout(() => {
          window.location.href = "/"; // Đá về Sảnh chờ (Lobby)
        }, 1000);
      } else {
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => setShowGoogleError(true), 800);
  };
    ...
    <div className={`min-h-screen bg-slate-50 flex items-center justify-center p-4 ${sans.className}`}>
      ...
      <div className="w-full max-w-4xl bg-white rounded-[2rem] shadow-2xl flex flex-col md:flex-row overflow-hidden min-h-[550px] border border-slate-100">
        ...
        <div className="w-full md:w-1/2 p-10 md:p-14 flex flex-col justify-center">
          ...
          {showGoogleError && (
            <div className="bg-rose-50 border border-rose-100 p-3 rounded-xl mb-4 animate-in fade-in slide-in-from-top-2">
               <p className="text-xs text-rose-500 font-bold text-center">Google authentication is under construction.</p>
            </div>
          )}
          ...
          <button onClick={handleGoogleLogin} className="w-full h-12 bg-white border-2 border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 rounded-xl font-bold transition-all flex items-center justify-center gap-3 active:scale-[0.98]">
