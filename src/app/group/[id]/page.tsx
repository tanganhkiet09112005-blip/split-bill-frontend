...
import {
  ...
  ArrowRight, Moon, Sun,
  Loader2, ArrowLeft, MapPin, PieChart as PieChartIcon, UserPlus
} from "lucide-react";
import { useRouter, useParams } from "next/navigation";

// ─── DYNAMIC IMPORTS (Chỉ chạy khi cần) ───────────────────────────────────────
...
export default function GroupDetailPage() {
  ...
  const addExpense = async (exp: Expense) => {
    try {
      const res = await fetch(`${API_URL}/expenses`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(exp) });
      if (res.ok) { loadData(); toast.success("Ghi nhận khoản chi!"); }
  ...
  return (
    <div className={`min-h-screen ${t.bg} transition-colors duration-300 pb-20`}>
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-8 flex flex-col gap-5">
        ...
        {/* EXPENSE FORM */}
        <div className={`p-5 rounded-2xl border ${t.card}`}>
           <h2 className={`text-sm font-bold flex items-center gap-2 ${t.text} mb-4`}><Plus size={16} className="text-indigo-600"/> Khoản chi mới</h2>
           <AddExpenseForm members={members} onAdd={addExpense} groupId={groupId} dark={dark} />
        </div>
          ...
          <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="min-h-[200px]">
            {tab === "expenses" && (
              <div className="flex flex-col gap-3">
                {expenses.length === 0 ? <p className="text-center text-xs text-slate-400 py-10 italic">Chưa có dữ liệu chi tiêu...</p> : 
                    ...
                    <div key={exp.id || idx} className={`flex items-center justify-between p-4 rounded-2xl border ${t.card}`}>
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {exp.latitude && <button onClick={() => setViewingExpense(exp)} className="flex items-center gap-0.5 text-indigo-500 font-black text-[10px] uppercase ml-1"><MapPin size={10} strokeWidth={3} /> Vị trí</button>}
                        </div>
                      </div>
                      <button onClick={() => exp.id && delExpense(exp.id)} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition"><Trash2 size={16} /></button>
                    </div>
                  ))
                }
              </div>
                ...
                {serverDebts.length === 0 ? <div className={`text-center py-10 rounded-2xl border ${t.card} bg-emerald-50/30 border-emerald-100`}><p className="text-emerald-600 font-black text-base uppercase">🎉 Đã sạch nợ!</p></div> : 
                  serverDebts.map((d, i) => (
                    <div key={i} className={`flex flex-col sm:flex-row items-center justify-between p-4 rounded-2xl border ${t.card} gap-4`}>
                      ...
                      <div className="flex items-center justify-between w-full sm:w-auto gap-4 sm:pl-4 sm:border-l border-slate-100 dark:border-slate-800">
        ...
        {isAddMemberOpen && (
          <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-black text-indigo-600 uppercase tracking-tighter italic">Add Friend</h3>
              ...
              <form onSubmit={handleAddMember}>
                <input type="text" placeholder="Tên bạn của Sếp..." value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} className="w-full h-12 px-4 mb-5 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 rounded-xl outline-none focus:border-indigo-500 font-bold" required autoFocus />
      ...
      <AnimatePresence>
        {viewingExpense && (
          <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="bg-white dark:bg-slate-900 rounded-3xl p-5 w-full max-w-sm shadow-2xl">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold dark:text-white line-clamp-1">Vị trí: {viewingExpense.description}</h3>
                <button onClick={() => setViewingExpense(null)} className="p-1 text-slate-400"><X size={20}/></button>
              </div>
              <div className="rounded-2xl overflow-hidden mb-4 border border-slate-100 dark:border-slate-800">
                <MapView latitude={viewingExpense.latitude!} longitude={viewingExpense.longitude!} />
              </div>
              <button onClick={() => setViewingExpense(null)} className="w-full py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-xs uppercase">Đóng</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
...
// ─── FORM THÊM CHI TIÊU (COMPACT DESIGN) ──────────────────────────────────────
function AddExpenseForm({ members, onAdd, groupId, dark }: { members: Member[], onAdd: (e: Expense) => void, groupId: string, dark: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAdd({ description: desc, amount: parseAmt(amount), paidBy: payer, groupId, splitType: "EQUAL", splitBetween: members.map(m => m.userId), latitude: location?.lat, longitude: location?.lng });
    setDesc(""); setAmount(""); setPayer(""); setLocation(null); setShowMap(false);
  };

  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <input placeholder="Hôm nay chi việc gì? (VD: Buffet sáng)" value={desc} onChange={e => setDesc(e.target.value)} className="w-full p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 font-bold text-sm text-slate-700 dark:text-slate-200" />
      <div className="flex flex-col sm:flex-row gap-3">
        <input placeholder="Số tiền (VND)" value={amount} onChange={e => setAmount(fmtInput(e.target.value))} className="w-full sm:w-1/2 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 font-black text-indigo-600 text-lg" />
      ...
      <div className="flex items-center justify-between gap-2">
        <button type="button" onClick={() => setShowMap(!showMap)} className={`flex items-center gap-1.5 text-[9px] font-black uppercase px-4 py-2 rounded-full border transition-all ${location ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'}`}>
          <MapPin size={12} /> {location ? "📍 Đã gắn vị trí" : "Gắn vị trí"}
        </button>
        <button type="submit" className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-black text-xs uppercase shadow-md flex items-center justify-center gap-2 transition-all">
          <Check size={14} strokeWidth={4} /> GHI NHẬN HÓA ĐƠN
        </button>
      </div>
      {showMap && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700"><MapPicker onLocationSelect={(lat, lng) => setLocation({lat, lng})} /></motion.div>}
    </form>
  );
}